...
ICPay supports verified ledgers across chains (IC ICRC and EVM). You can fetch lists (with or without price data), resolve canister IDs by symbol, and read detailed metadata. {{ className: 'lead' }}

## Overview

- Each ledger provides `id`, `name`, `symbol`, `shortcode?`, `canisterId`, `decimals`, `fee`, `verified`, and optional price data.
...
- `chainId` refers to the underlying chain (ICPay backend UUID) associated to the ledger.
...
Widgets fetch verified ledgers automatically. You can constrain choices via multichain filters in the widget config:
