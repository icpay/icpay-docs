...
- Built-in wallet selector for Plug, Internet Identity, and Oisy.
...
// Prompt the user to connect (optionally specify a provider id: 'plug' | 'ii' | 'oisy')
await walletSelect.connect()
...
- `openOisyInNewTab?` (boolean) — If `true`, Oisy opens in a new tab for connect/transfer. Default: `false` (popup). Useful if your site/browser blocks popups or breaks window.opener.
- `theme?`
