...
const start = await icpay.sendFundsUsd({
  usdAmount: 25,
  ledgerCanisterId: icpLedger.canisterId,
...
- `awaitServerNotification` (boolean): Await server-side notify step in `sendFunds` and `sendFundsUsd`.
...
| `icpay-sdk-transaction-completed` | When the transaction reaches completed | Same as above (final status `completed`) |
| `icpay-sdk-transaction-failed` | When the transaction reaches failed | Same as above (final status `failed`). Also emitted when amounts mismatch. |
| `icpay-sdk-transaction-mismatched` | When on-ledger paid amount differs from requested | `{ ...TransactionResponse, requestedAmount?: string, paidAmount?: string }` |
| `icpay-sdk-connect-wallet` | When a connect-wallet flow is surfaced (if used by integrator) | Implementation-defined (string/provider or structured data) |
| `icpay-sdk-method-start` | At the start of an SDK method | `{ name: string, args?: any }` |
| `icpay-sdk-method-success` | On successful method completion | `{ name: string, result?: any }` |
| `icpay-sdk-method-error` | On method-level error (plus `icpay-sdk-error`) | `{ name: string, error: any }` |

<div className="not-prose">
  <Button href="/widget" arrow="right" variant="text">
