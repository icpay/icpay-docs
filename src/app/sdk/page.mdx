...
const tx = await icpay.createPayment({
  ...
  // chainId: '<uuid>', // Optional: defaults to Internet Computer when omitted
})
```
...
```ts {{ title: 'Onramp intent polling (e.g., Transak order lifecycle)' }}
// After starting an onramp intent, poll until terminal status:
const ctrl = icpay.notifyPaymentIntentOnRamp({ paymentIntentId: '<intentId>', intervalMs: 5000 })
// ...later, you may stop polling:
// ctrl.stop()
```
...
const all = await icpay.getExternalWalletBalances({
  ...
  ledgerShortcodes: ['ckusdc'], // optional filter
})
