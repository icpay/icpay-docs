...
const widgets: Array<Widget> = [
  {
    href: '/widget/components/tip-jar',
    name: 'Tip Jar',
    description: 'Accept tips and donations from your audience with a simple, elegant interface.',
    icon: FaceSmileIcon,
    pattern: {
      y: 16,
      squares: [
  ...
  {
    href: '/widget/components/premium-content',
    name: 'Premium Content',
    description: 'Sell access to exclusive content with secure paywall protection.',
    icon: BookIcon,
    pattern: {
      y: -6,
      squares: [
  ...
  {
    href: '/widget/components/coffee-shop',
    name: 'Coffee Shop',
    description: 'Create a digital storefront for selling digital products and services.',
    icon: CartIcon,
    pattern: {
      y: 32,
      squares: [
  ...
  {
    href: '/widget/components/donation-thermometer',
    name: 'Donation Thermometer',
    description: 'Track and display progress towards fundraising goals with visual feedback.',
    icon: BoltIcon,
    pattern: {
      y: 22,
      squares: [[0, 1]],
...
function WidgetIcon({ icon: Icon }: { icon: Widget['icon'] }) {
  return (
...
function Widget({ widget }: { widget: Widget }) {
  ...
  return (
    ...
    >
      <WidgetPattern {...widget.pattern} mouseX={mouseX} mouseY={mouseY} />
...
export function Widgets() {
  return (
    <div className="my-16 xl:max-w-none">
      <Heading level={2} id="widgets">
        Widget Components
      </Heading>
      <div className="not-prose mt-4 grid grid-cols-1 gap-8 border-t border-zinc-900/5 pt-10 sm:grid-cols-2 xl:grid-cols-4 dark:border-white/5">
        {widgets.map((widget) => (
          <Widget key={widget.href} widget={widget} />
        ))}
      </div>
    </div>
  )
